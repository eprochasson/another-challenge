...
var interval = {
    ...
    intervals2str: function(intervals){
        ...
        _.each(intervals, function(interval){
            if(interval && interval.length == 1 || interval[0] == interval[1]){
    ...
    substract: function(inter1, inter2){
        ...
        inter1[0] = parseInt(inter1[0],10);
        inter1[1] = parseInt(inter1[1],10);
        inter2[0] = parseInt(inter2[0],10);
        inter2[1] = parseInt(inter2[1],10);
    ...
    merge: function(intervals, exclude){
        ...
        _.each(intervals, function(interval){
            if(interval && interval.length == 1){ // insertion of single value
                interval = [parseInt(interval[0],10), parseInt(interval[0],10)]; // transform it into a doublet.
            } else {
                interval = [parseInt(interval[0],10), parseInt(interval[1],10)];
            }
        ...
        for(var i = 0 ; i < results.length ; i++){
            if(i>0){
                if(results[i][0] <= max+margin){
                    ...
                    delete results[i]; // kill the current, useless value.
                } else {
    ...
    // return the smallest value in a set of intervals (for the infamous
    // +S6-0:1130-1500,1800-2230;1-5:1200-1430,1800-2230
    min: function(intervals){
        var min = -1;
        _.each(intervals, function(interval){
            ...
            if(min === -1 || Math.min(parseInt(interval[0],10), parseInt(interval[1],10)) < min){
                min = Math.min(parseInt(interval[0],10), parseInt(interval[1],10));
            }
        });
        return min;
    },

    // normalize the way intervals are sorted
    // [[5,6], [1]] -> [ [1], [5,6] ]
    normalize: function(intervals){
        var self = this;
        if(typeof intervals === 'string'){
            intervals = this.str2intervals(intervals); // I should refactor this whole class to include this.
        }

        return _.sortBy(intervals, function(n){ return self.min(n)});
    }
};
