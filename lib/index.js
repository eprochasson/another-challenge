...
//                Thursday
    ...
    merge: function(values){
        ...
        _.each(dates, function(inter1, index1){
            _.each(dates, function(inter2, index2){
                ...
                if(index1 !== index2 && result[inter2]){
                    ...
                    if(self.intervals_iscontained(self.text2intervals(inter1)[0], self.text2intervals(inter2)[0])){
                        substitution = true;
                        result[inter1] = result[inter1].concat(result[inter2]);
                        console.log(inter1, 'isin', inter2);
                        // split the previous interval to account for the copy and avoid duplicated content.
                        console.log('sub', self.text2intervals(inter2)[0], 'to ', self.text2intervals(inter1)[0]);
                        intervals = self.intervals2text(self.intervals_substract(self.text2intervals(inter2)[0], self.text2intervals(inter1)[0]));
                ...
                if(substitution){
                    result[inter2] = null;
                    console.log("RESULT", inter2, result);
                }
            })
        });
        ...
        _.each(_.keys(result), function(k){
            if(result[k]){
                final_result[k] = self.intervals_merge(result[k], true);
            }
        });
    ...
    // intervat2text: Array -> string
    // take something like [ [3], [5,6] ] and return "3,5-6"
    intervals2text: function(intervals){
        var a_string = [];
    ...
    // text2interval: string -> Array
    // take something like "3,5-6" and return  [ [3], [5,6] ]
    text2intervals: function(str){
        var str_array;
        var results = [];
...
//
//console.log( hours.intervals_substract([0, 6], [0, 3]));// -> [[4,6]]
//console.log( hours.intervals_substract([0, 6], [4, 6])); // -> [[0, 3]]
//console.log( hours.intervals_substract([0, 6], [4, 7])); // -> [[0, 3]]
//console.log( hours.intervals_substract([0, 6], [2, 3])); // -> [[0,1], [4,6]
//console.log( hours.intervals_substract([0,6], [0,6])); // -> []
console.log( hours.intervals_substract([0,6], [0,4])); // -> []
